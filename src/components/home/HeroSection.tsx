...
const dashboards = [
  {
    ...
    metrics: { spend: "₹2.4L", roas: "3.8x", leads: "1,240", cpc: "₹12.50" },
  },
  {
    label: "Google Ads",
    icon: BarChart3,
    color: "from-green-600 to-emerald-400",
    metrics: { spend: "₹1.8L", roas: "4.2x", leads: "890", cpc: "₹18.20" },
  },
  {
    label: "AI Automation",
    icon: Zap,
    color: "from-purple-600 to-violet-400",
    metrics: { workflows: "45", saved: "120 hrs", emails: "8,500", conversion: "22%" },
  },
  {
    label: "Analytics",
    icon: TrendingUp,
    color: "from-orange-500 to-amber-400",
    metrics: { users: "24.5K", sessions: "38.2K", bounce: "32%", revenue: "₹18.6L" },
  },
];
          ...
          >
            <div className="grid grid-cols-2 gap-3">
              {dashboards.map((d, i) => (
                <motion.div
                  key={i}
                  className="relative rounded-xl border border-primary-foreground/10 bg-primary-foreground/5 backdrop-blur-sm p-4 cursor-pointer overflow-hidden"
                        ...
                        <div className="h-6 mt-2 rounded bg-primary-foreground/5 flex items-center justify-center">
                          <div className="flex gap-1">
                            {[40, 65, 45, 80, 55, 70, 50].map((h, j) => (
                              <div key={j} className="w-1.5 bg-primary-foreground/20 rounded-t" style={{ height: `${h * 0.3}px` }} />
                            ))}
                          </div>
                        </div>
                      </div>
