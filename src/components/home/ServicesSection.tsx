...
import { Target, BarChart3, TrendingUp, Film, PenTool, Share2, Monitor, ArrowRight, Star } from "lucide-react";
import SectionHeading from "@/components/SectionHeading";
import { useSetting } from "@/hooks/useSiteSettings";

const iconMap: Record<string, any> = { Target, BarChart3, TrendingUp, Film, PenTool, Share2, Monitor, Star };
  ...
  return (
    <section className="section-padding bg-background">
      <div className="container mx-auto">
        <SectionHeading badge="What We Do" title="AI-Powered Services That Drive Growth" subtitle="From strategy to execution, we combine AI intelligence with human creativity to handle every aspect of your digital marketing." />
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
                <Link to={`/services/${service.slug}`} className="group block p-8 rounded-xl border border-border bg-card hover:border-primary/30 hover:shadow-lg transition-all duration-300 h-full">
                  <div className="w-12 h-12 rounded-lg bg-secondary flex items-center justify-center mb-5 group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
                    <IconComp size={24} className="text-primary group-hover:text-primary-foreground" />
                  </div>
                  <h3 className="font-display text-xl font-semibold text-card-foreground mb-2">{service.title}</h3>
                  <p className="text-muted-foreground text-sm leading-relaxed mb-4">{service.desc}</p>
                  <span className="inline-flex items-center text-sm font-medium text-primary">Learn More <ArrowRight size={16} className="ml-1" /></span>
                </Link>
              </motion.div>
            );
          })}
