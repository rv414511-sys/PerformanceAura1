...
const categoryIcons: Record<string, any> = { PDF: FileText, Guide: BookOpen, Checklist: CheckSquare, Template: Layout };
...
const Resources = () => {
  ...
  const { value: pageData } = useSetting("page_resources");

  const page = {
    badge: "Free Resources",
    title: "Marketing Resources",
    titleHighlight: "To Accelerate Growth",
    subtitle: "Download free guides, templates, and checklists crafted by our marketing experts. Sign up to access all resources.",
  ...
  const handleDownload = async (resource: any) => {
    if (!user) { toast({ title: "Sign up required", description: "Please create an account to download.", variant: "destructive" }); return; }
    if (resource.file_url) {
      await supabase.from("resource_downloads").insert({ user_id: user.id, resource_id: resource.id });
      window.open(resource.file_url, "_blank");
    } else { toast({ title: "Coming soon!", description: "This resource will be available shortly." }); }
  };
              ...
              return (
                <motion.div key={r.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-border">
                    ...
                    {user ? (
                      <Button size="sm" onClick={() => handleDownload(r)}><Download size={14} className="mr-1" /> Download</Button>
                    ) : (
                      <Button size="sm" asChild><Link to="/signup"><Lock size={14} className="mr-1" /> Sign Up to Access</Link></Button>
                    )}
                  </div>
                </motion.div>
              );
