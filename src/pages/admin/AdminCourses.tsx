    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Courses add karein — users purchase kar sakenge</p>
        </div>
        ...
        <div className="p-6 rounded-xl border border-border bg-card mb-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <Input placeholder="Course ka naam" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            </div>
            <div className="space-y-1">
      ...
      {isLoading ? <p className="text-muted-foreground">Loading...</p> : (
        <div className="space-y-3">
          ...
          {courses?.length === 0 && <p className="text-muted-foreground text-center py-8">Koi course nahi hai abhi.</p>}
        </div>
      )}
    </div>
  );
